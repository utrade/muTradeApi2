...
void SBA::Bidding::bidDriver(void *params)
{
  ...
  obj->reqStartAlgo(true, false, true);
...
{
  API2::COMMON::MktData *_mkData = reqQryUpdateMarketData( _params.symbol );
/*
  if( _mkData->getIsPreTrade() )
  {
    DEBUG_VARSHOW( reqQryDebugLog(), "PreTradeDepthCount: ",_mkData->getPreTradeDepthCount() );
    DEBUG_VARSHOW( reqQryDebugLog(), "PreTradeOrderQty: ",_mkData->getPreTradeOrderQty() );
    DEBUG_VARSHOW( reqQryDebugLog(), "PreTradeOrderMode: ",_mkData->getPreTradeOrderMode() );
    DEBUG_VARSHOW( reqQryDebugLog(), "PreTradeAveragePrice ",_mkData->getPreTradeAveragePrice() );
  }
  else 
  {
   DEBUG_VARSHOW( reqQryDebugLog(), "IsPreTrade: ",_mkData.getIsPreTrade() );
  }
*/
  onDefaultEvent();
}
...
void SBA::Bidding::onProcessPreTradeConfirmation(API2::PreTradeConfirmation& preTradeConfirmation)
{
  DEBUG_MESSAGE( reqQryDebugLog(), "In PretradeConfirmation......................" );  
  DEBUG_MESSAGE( reqQryDebugLog(), preTradeConfirmation.getString().c_str() );
}
