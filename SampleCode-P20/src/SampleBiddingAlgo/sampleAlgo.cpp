  ...
  _terminateCheck(false)
{
  ...
  if(!registerAlgoDetail(_algoId))
  {
    DEBUG_VARSHOW(reqQryDebugLog(),"Invalid Algo Id = " , _algoId );
    terminateStrategyComment(API2::CONSTANTS::RSP_StrategyComment_STRATEGY_ERROR_STATE);
    return;
  }
...
bool SBA :: Bidding::readParamsFromFile()
{
   namespace bpo=boost::program_options;
  try
  {
    bpo::options_description config("sample algo");
    config.add_options()
      ("ALGO_ID" , bpo::value<SIGNED_LONG> (&_algoId)->default_value(0),
       "Algo Id ")
      ;

    std::string configFile = "./config/sample.txt";
    ...
    {
      DEBUG_MESSAGE(reqQryDebugLog(), "File not found while reading parameters");
      return false;
    }
    return true;
  }
  ...
  {
    DEBUG_VARSHOW(reqQryDebugLog(),"Bad Options:", err.what());
    return false;
  }
  catch(std::exception &err)
  {
    DEBUG_VARSHOW(reqQryDebugLog(),"Caught std::exception", err.what());
    return false;
  }
}
...
void SBA::Bidding::mapModifiedParameters()
{
    ...
    {
  _bidOrderWrapper = API2::CUSTOM_COMMON::OrderWrapper(_bidInstrument ,(int)_params.orderMode , this , _params.account );
}
